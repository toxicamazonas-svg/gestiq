...
def _foto_b64(path):
    """PNG circular de 128 px en base64 a partir de cualquier imagen."""
...
def _foto_ctk(b64, size):
    """CTkImage circular desde el base64 guardado; None si falla."""
    try:
        from PIL import Image
...
GLASS  = ("#E2E2EA", "#34344A")   # borde "vidrio" de tarjetas (estilo web)
...
def fade(w, c1, c2, final=None, pasos=9, dt=16):
    """Transición fluida de color en cualquier widget CTk (ease-out)."""
    if getattr(w, "_fade_id", None):
        try: w.after_cancel(w._fade_id)
        except Exception: pass
        w._fade_id = None
    def paso(i=0):
        e = 1 - (1 - i / pasos) ** 3
        try: w.configure(fg_color=_hex_mix(c1, c2, e))
        except Exception: return
        if i < pasos:
            w._fade_id = w.after(dt, paso, i + 1)
        else:
            w._fade_id = None
            if final: w.configure(fg_color=final)
    paso()

def hover_fluido(btn, base, hov):
    """Hover animado: sustituye el cambio instantáneo de color de CTk."""
    btn.configure(hover=False)
    def entra(_):
        if btn.cget("state") == "normal": fade(btn, _col(base), _col(hov), pasos=6, dt=14)
    def sale(_):
        if btn.cget("state") == "normal": fade(btn, _col(hov), _col(base), pasos=6, dt=14)
    btn.bind("<Enter>", entra)
    btn.bind("<Leave>", sale)
...
class App(ctk.CTk):
    ...
    def __init__(self):
        super().__init__()
        _init_fonts()
        # Tema preferido del último usuario, antes de construir la interfaz
        try:
            t = _prefs_get(licencia.ultimo_email() if licencia else "").get("tema")
            if t:
                ctk.set_appearance_mode("dark" if t == "oscuro" else "light")
        except Exception:
            pass
        self.title("Gestiq")
        ...
        try: self.attributes("-alpha", 0.0)   # aparición suave de la ventana
        except Exception: pass
    ...
    def _build(self):
        ...
        theme_box = ctk.CTkFrame(hdr, fg_color=CARD2, corner_radius=16,
                                 border_width=1, border_color=GLASS)
        ...
        if ctk.get_appearance_mode() == "Dark":
            ...
            self.sw_theme.configure(text="Claro")

        # Panel de sesión (se muestra solo con sesión iniciada)
        self.sess_box = ctk.CTkFrame(hdr, fg_color=CARD2, corner_radius=16,
                                     border_width=1, border_color=GLASS)
        # Avatar con la inicial de la cuenta + punto "en línea"
        self._sess_avatar = ctk.CTkFrame(self.sess_box, fg_color=ACCENT,
                                         corner_radius=13, width=26, height=26)
        self._sess_avatar.pack(side="left", padx=(8, 8), pady=6)
        self._sess_avatar.pack_propagate(False)
        self._sess_ini = ctk.CTkLabel(self._sess_avatar, text="", font=F(11, True),
                                      text_color="#FFFFFF")
        self._sess_ini.place(relx=.5, rely=.5, anchor="center")
        info = ctk.CTkFrame(self.sess_box, fg_color="transparent")
        info.pack(side="left", padx=(0, 10), pady=4)
        self.lbl_sess = ctk.CTkLabel(info, text="", font=F(12, True),
                                     text_color=TX, anchor="w", height=14)
        self.lbl_sess.pack(anchor="w")
        ctk.CTkLabel(info, text="●  Sesión activa", font=F(11),
                     text_color=ACCENT, anchor="w", height=12).pack(anchor="w")
        ctk.CTkButton(self.sess_box, text="⚙", font=F(14), width=30, height=28,
                      corner_radius=14, fg_color="transparent",
                      hover_color=BORDER, text_color=TM,
                      command=self._abrir_prefs).pack(side="left", pady=6)
        ctk.CTkFrame(self.sess_box, fg_color=BORDER, width=1,
                     height=24).pack(side="left", padx=(8, 0), pady=8)
        btn_out = ctk.CTkButton(self.sess_box, text="⏻  Cerrar sesión", font=F(11),
                                width=116, height=28, corner_radius=14,
                                fg_color=CARD2, border_width=1,
                                border_color=BORDER, text_color=TM, hover=False,
                                command=self._lic_cerrar_sesion)
        btn_out.pack(side="left", padx=(10, 8), pady=6)
        # Hover "peligro": fundido suave a rojo para avisar que es una salida
        _OUT_BG = ("#FBE7E8", "#3A2026")
        def _out_in(_):
            fade(btn_out, _col(CARD2), _col(_OUT_BG), pasos=6, dt=14)
            btn_out.configure(text_color=ERR, border_color=ERR)
        def _out_out(_):
            fade(btn_out, _col(_OUT_BG), _col(CARD2), pasos=6, dt=14, final=CARD2)
            btn_out.configure(text_color=TM, border_color=BORDER)
        btn_out.bind("<Enter>", _out_in)
        btn_out.bind("<Leave>", _out_out)
        ...
        # Indicador deslizante del módulo activo (estilo glider de la web)
        self._nav_ind = ctk.CTkFrame(side, fg_color=ACCENT, corner_radius=3,
                                     width=4, height=26)
    ...
    def _show(self, key):
        ...
        self.after(50, lambda: self._nav_indicar(key))
    ...
    def _fade(self, btn, c1, c2, final=None, pasos=9, dt=16):
        ...
        fade(btn, c1, c2, final=final, pasos=pasos, dt=dt)

    def _aparecer(self, i=0, pasos=14):
        """Fade-in de la ventana al abrir."""
        e = 1 - (1 - i / pasos) ** 3
        try: self.attributes("-alpha", e)
        except Exception:
            try: self.attributes("-alpha", 1.0)
            except Exception: pass
            return
        if i < pasos: self.after(15, self._aparecer, i + 1)

    def _slide(self, w, y_to, pasos=12, dt=14):
        """Desliza un widget colocado con place hasta y_to (ease-out)."""
        try: y0 = w.winfo_y()
        except Exception: return
        if getattr(w, "_slide_id", None):
            try: w.after_cancel(w._slide_id)
            except Exception: pass
            w._slide_id = None
        def paso(i=0):
            e = 1 - (1 - i / pasos) ** 3
            try: w.place_configure(y=round(y0 + (y_to - y0) * e))
            except Exception: return
            if i < pasos:
                w._slide_id = w.after(dt, paso, i + 1)
            else:
                w._slide_id = None
        paso()

    def _nav_indicar(self, key):
        """Mueve el indicador lateral hasta el módulo activo."""
        b = self._nav.get(key)
        ind = getattr(self, "_nav_ind", None)
        if not b or not ind: return
        self.update_idletasks()
        y = b.winfo_y() + max(0, (b.winfo_height() - 26) // 2)
    ...
    def _lic_ok(self, s, r=None):
        ...
        p = _prefs_get(em)
        if p.get("tema"):
            self._aplicar_tema(p["tema"])
        self._chip_actualizar()
        self.sess_box.pack(side="right", padx=(0, 8), pady=12)
        if p.get("modulo") in self._mods_permitidos():
            self._show(p["modulo"])
        if not p.get("nombre"):
            self.after(200, self._pedir_nombre)

    # ── Perfil y preferencias del usuario ────────────────────────────────────
    def _chip_actualizar(self):
        """Pinta nombre y avatar (foto o inicial) en el chip de sesión."""
        em = (self._lic or {}).get("email", "")
        p = _prefs_get(em)
        nombre = p.get("nombre") or em
        self.lbl_sess.configure(text=nombre)
        self.title(f"Gestiq — {nombre}" if nombre else "Gestiq")
        img = _foto_ctk(p["foto"], 26) if p.get("foto") else None
        if img:
            self._sess_ini.configure(text="", image=img)
            self._sess_avatar.configure(fg_color="transparent")
        else:
            self._sess_ini.configure(text=(nombre[:1] or "?").upper(), image=None)
            self._sess_avatar.configure(fg_color=ACCENT)

    def _aplicar_tema(self, tema):
        """Aplica 'oscuro'/'claro' y sincroniza el interruptor del encabezado."""
        dark = tema == "oscuro"
        if (ctk.get_appearance_mode() == "Dark") == dark:
            return
        if dark:
            self.sw_theme.select()
        else:
    ...
    def _pedir_nombre(self):
        """Primer inicio de la cuenta: pide el nombre de usuario (obligatorio)."""
        em = (self._lic or {}).get("email", "")
        if not em:
            return
        dlg = ctk.CTkToplevel(self)
        dlg.title("Crea tu perfil")
        dlg.configure(fg_color=BG)
        dlg.resizable(False, False)
        dlg.transient(self)
        dlg.protocol("WM_DELETE_WINDOW", lambda: None)   # nombre obligatorio
        self._centrar_top(dlg, 380, 440)
        ...
        ctk.CTkLabel(card, text=f"{saludo} 👋", font=F(19, True),
                     text_color=TX).pack(pady=(24, 2))
        ctk.CTkLabel(card, text="¿Cómo quieres que te llamemos?",
                     font=F(12), text_color=TM).pack()

        av = ctk.CTkFrame(card, fg_color=ACCENT, corner_radius=36,
                          width=72, height=72)
        av.pack(pady=(18, 8))
        av.pack_propagate(False)
        lbl_av = ctk.CTkLabel(av, text="?", font=F(26, True),
                              text_color="#FFFFFF")
        lbl_av.place(relx=.5, rely=.5, anchor="center")

        foto_sel = [None]
        def elegir_foto():
            ruta = filedialog.askopenfilename(
                parent=dlg, title="Elige tu foto",
                filetypes=[("Imágenes", "*.png *.jpg *.jpeg *.webp *.bmp *.gif")])
            if not ruta:
                return
            try:
                foto_sel[0] = _foto_b64(ruta)
                lbl_av.configure(text="", image=_foto_ctk(foto_sel[0], 72))
                av.configure(fg_color="transparent")
            except Exception:
                lbl_est.configure(text="No se pudo cargar esa imagen")
        ctk.CTkButton(card, text="Elegir foto (opcional)", width=180, height=30,
                      corner_radius=15, font=F(12), fg_color=CARD2,
                      hover_color=BORDER, text_color=TX, border_width=1,
                      border_color=BORDER, command=elegir_foto).pack()

        ent = ctk.CTkEntry(card, width=280, height=42, corner_radius=12,
                           border_width=1, placeholder_text="Tu nombre de usuario",
                           font=F(13), fg_color=CARD2, border_color=BORDER,
                           text_color=TX)
        ent.pack(pady=(16, 4))
        lbl_est = ctk.CTkLabel(card, text="", font=F(11), text_color=ERR)
        lbl_est.pack()
        ...
        def guardar():
            ...
            if not n:
                return
            _prefs_set(em, nombre=n, foto=foto_sel[0])
            self._chip_actualizar()
            try: dlg.grab_release()
            except Exception: pass
            dlg.destroy()
        btn = ctk.CTkButton(card, text="Guardar y continuar", width=280,
                            height=42, corner_radius=21, font=F(14, True),
                            fg_color=ACCENT, hover_color=ACCENT_H,
                            state="disabled", command=guardar)
        btn.pack(pady=(4, 0))

        def al_escribir(*_):
            n = ent.get().strip()
            btn.configure(state="normal" if n else "disabled")
            if foto_sel[0] is None:   # la inicial sigue al nombre en vivo
                lbl_av.configure(text=(n[:1] or "?").upper())
        ent.bind("<KeyRelease>", al_escribir)
        ent.bind("<Return>", lambda *_: guardar())

        dlg.after(180, lambda: (dlg.lift(), dlg.grab_set(), ent.focus_set()))
        ctk.CTkLabel(card, text="Podrás cambiarlo luego en  ⚙  Preferencias",
                     font=F(11), text_color=TM).pack(pady=(12, 0))

    def _abrir_prefs(self):
        """Ventana ⚙ Preferencias: perfil, apariencia e inicio."""
        em = (self._lic or {}).get("email", "")
        if not em:
            return
        if getattr(self, "_prefs_win", None) is not None and \
                self._prefs_win.winfo_exists():
            self._prefs_win.lift(); self._prefs_win.focus_force(); return
        p = _prefs_get(em)
        ...
        win.transient(self)
        self._centrar_top(win, 430, 596)

        ctk.CTkLabel(win, text="⚙  Preferencias", font=F(19, True),
                     text_color=TX).pack(anchor="w", padx=24, pady=(18, 8))

        def tarjeta(titulo):
            c = ctk.CTkFrame(win, fg_color=CARD, corner_radius=16,
                             border_width=1, border_color=GLASS)
            c.pack(fill="x", padx=20, pady=6)
            ctk.CTkLabel(c, text=titulo, font=F(11, True),
                         text_color=TM).pack(anchor="w", padx=16, pady=(12, 4))
            return c
        ...
        av = ctk.CTkFrame(fila, fg_color=ACCENT, corner_radius=28,
                          width=56, height=56)
        av.pack(side="left")
        av.pack_propagate(False)
        lbl_av = ctk.CTkLabel(av, text="?", font=F(20, True),
                              text_color="#FFFFFF")
        lbl_av.place(relx=.5, rely=.5, anchor="center")

        def pintar_avatar(b64, nombre):
            img = _foto_ctk(b64, 56) if b64 else None
            if img:
                lbl_av.configure(text="", image=img)
                av.configure(fg_color="transparent")
            else:
                lbl_av.configure(text=(nombre[:1] or "?").upper(), image=None)
                av.configure(fg_color=ACCENT)

        botones = ctk.CTkFrame(fila, fg_color="transparent")
        botones.pack(side="left", padx=14)
        foto_sel = ["keep"]                  # keep | "" (quitar) | base64 nueva
        def elegir():
            ruta = filedialog.askopenfilename(
                parent=win, title="Elige tu foto",
                filetypes=[("Imágenes", "*.png *.jpg *.jpeg *.webp *.bmp *.gif")])
            if not ruta:
                return
            try:
                foto_sel[0] = _foto_b64(ruta)
                pintar_avatar(foto_sel[0], ent_nom.get().strip())
            except Exception:
                lbl_est.configure(text="No se pudo cargar esa imagen",
                                  text_color=ERR)
        def quitar():
            foto_sel[0] = ""
            pintar_avatar(None, ent_nom.get().strip())
        ctk.CTkButton(botones, text="Cambiar foto…", width=130, height=28,
                      corner_radius=14, font=F(12), fg_color=CARD2,
                      hover_color=BORDER, text_color=TX, border_width=1,
                      border_color=BORDER, command=elegir).pack(anchor="w")
        ctk.CTkButton(botones, text="Quitar foto", width=130, height=24,
                      corner_radius=12, font=F(11), fg_color="transparent",
                      hover_color=CARD2, text_color=TM,
                      command=quitar).pack(anchor="w", pady=(4, 0))

        ctk.CTkLabel(c1, text="Nombre de usuario", font=F(12),
                     text_color=TM).pack(anchor="w", padx=16, pady=(10, 2))
        ent_nom = ctk.CTkEntry(c1, height=38, corner_radius=12, border_width=1,
                               font=F(13), fg_color=CARD2, border_color=BORDER,
                               text_color=TX)
        ent_nom.pack(fill="x", padx=16)
        ent_nom.insert(0, p.get("nombre", ""))
        ctk.CTkLabel(c1, text=f"Cuenta: {em}", font=F(11),
                     text_color=TM).pack(anchor="w", padx=16, pady=(6, 12))
        pintar_avatar(p.get("foto"), p.get("nombre") or em)

        # ── Apariencia ───────────────────────────────────────────────────────
        c2 = tarjeta("APARIENCIA")
        ctk.CTkLabel(c2, text="Tema al iniciar la aplicación", font=F(12),
                     text_color=TX).pack(anchor="w", padx=16)
        seg_tema = ctk.CTkSegmentedButton(
            c2, values=["🌙  Oscuro", "☀️  Claro"], font=F(12), height=32,
            fg_color=CARD2, selected_color=ACCENT, selected_hover_color=ACCENT_H,
            unselected_color=CARD2, unselected_hover_color=BORDER,
            text_color="#FFFFFF" if ctk.get_appearance_mode() == "Dark" else None)
        seg_tema.pack(anchor="w", padx=16, pady=(6, 12))
        tema0 = p.get("tema") or ("oscuro" if ctk.get_appearance_mode() == "Dark"
                                  else "claro")
        seg_tema.set("🌙  Oscuro" if tema0 == "oscuro" else "☀️  Claro")

        # ── Inicio ───────────────────────────────────────────────────────────
        c3 = tarjeta("INICIO")
        ctk.CTkLabel(c3, text="Módulo al abrir sesión", font=F(12),
                     text_color=TX).pack(anchor="w", padx=16)
        seg_mod = ctk.CTkSegmentedButton(
            c3, values=["🔍  IMAGINE", "🛡  GUARDIÁN"], font=F(12), height=32,
            fg_color=CARD2, selected_color=ACCENT, selected_hover_color=ACCENT_H,
            unselected_color=CARD2, unselected_hover_color=BORDER)
        seg_mod.pack(anchor="w", padx=16, pady=(6, 12))
        seg_mod.set("🛡  GUARDIÁN" if p.get("modulo") == "guardian"
                    else "🔍  IMAGINE")

        # ── Guardar ──────────────────────────────────────────────────────────
        lbl_est = ctk.CTkLabel(win, text="", font=F(12), text_color=ACCENT)
        def guardar():
            n = ent_nom.get().strip()
            if not n:
                lbl_est.configure(text="El nombre no puede estar vacío",
                                  text_color=ERR)
                return
            kw = dict(nombre=n,
                      tema="oscuro" if "Oscuro" in seg_tema.get() else "claro",
                      modulo="guardian" if "GUARDIÁN" in seg_mod.get()
                             else "imagine")
            if foto_sel[0] != "keep":
                kw["foto"] = foto_sel[0] or None
            _prefs_set(em, **kw)
            self._aplicar_tema(kw["tema"])
            if kw["modulo"] in self._mods_permitidos():
                self._show(kw["modulo"])
            self._chip_actualizar()
            lbl_est.configure(text="✓  Cambios guardados", text_color=ACCENT)
            win.after(900, win.destroy)
        btn = ctk.CTkButton(win, text="Guardar cambios", height=42,
                            corner_radius=21, font=F(14, True),
                            fg_color=ACCENT, hover_color=ACCENT_H,
                            command=guardar)
        btn.pack(fill="x", padx=20, pady=(10, 4))
        hover_fluido(btn, ACCENT, "#00D55F")
        lbl_est.pack()
        ctk.CTkLabel(win, text=f"Gestiq {self.VERSION}", font=F(11),
                     text_color=TM).pack(pady=(2, 10))
        win.after(180, lambda: (win.lift(), win.focus_force()))
    ...
    # ── Overlay de acceso: fondo con halos + tarjeta animada ────────────────
    def _lock_glow(self, cv):
        """Dibuja halos de luz suaves en el fondo (estilo de la web)."""
        try:
        ...
        def halo(cx, cy, r, color, t_max, steps=22):
            for i in range(steps, 0, -1):
                t = t_max * (1 - i / steps) ** 2
                rr = r * i / steps
                cv.create_oval(cx - rr, cy - rr, cx + rr, cy + rr,
                               fill=_hex_mix(bg, color, t), width=0)
        halo(w * .16, h * .10, max(w, h) * .42, ACCENT,
             .20 if dark else .12)
        halo(w * .86, h * .94, max(w, h) * .38, "#7C5CFF",
             .16 if dark else .09)

    @staticmethod
    def _lock_slide(card, y0=.49, y1=.45, pasos=12, dt=14):
        """Entrada de la tarjeta: deslizamiento suave hacia arriba (ease-out)."""
        def paso(i=0):
            e = 1 - (1 - i / pasos) ** 3
            try: card.place_configure(rely=y0 + (y1 - y0) * e)
            except Exception: return
            if i < pasos:
                card.after(dt, paso, i + 1)
        paso()
    ...
    def _lock_mostrar(self, modo, msg="", reintentar=False):
        ...
        # Fondo decorativo (se redibuja al cambiar el tamaño de la ventana)
        cv = tk.Canvas(self._lock, highlightthickness=0, bd=0)
        cv.pack(fill="both", expand=True)
        def _redraw(_=None):
            if getattr(cv, "_rid", None):
                try: cv.after_cancel(cv._rid)
                except Exception: pass
            cv._rid = cv.after(50, lambda: self._lock_glow(cv))
        cv.bind("<Configure>", _redraw)

        card = ctk.CTkFrame(self._lock, fg_color=CARD, corner_radius=22,
                            border_width=1, border_color=GLASS)
        card.place(relx=.5, rely=.49, anchor="center")
        self._lock_slide(card)
        inner = ctk.CTkFrame(card, fg_color="transparent")
        inner.pack(padx=44, pady=34)

        # Marca: insignia IP (roja si el acceso está bloqueado) + nombre
        badge = ctk.CTkFrame(inner, corner_radius=14, width=46, height=46,
                             fg_color=ERR if modo == "bloqueado" else ACCENT)
        badge.pack()
        badge.pack_propagate(False)
        ctk.CTkLabel(badge, text="!" if modo == "bloqueado" else "IP",
                     font=F(16, True), text_color="#FFFFFF"
                     ).place(relx=.5, rely=.5, anchor="center")
        ctk.CTkLabel(inner, text="Gestiq", font=F(21, True),
                     text_color=TX).pack(pady=(8, 0))

        # Pie fijo del overlay con la versión
        ctk.CTkLabel(self._lock,
                     text=f"Gestiq {self.VERSION}  ·  Automatización IMAGINE y GUARDIÁN",
                     font=F(11), text_color=TM).place(relx=.5, rely=.965,
                                                      anchor="center")

        if modo == "cargando":
            ctk.CTkLabel(inner, text="Verificando licencia…", font=F(15, True),
                         text_color=TX).pack(pady=(14, 2))
            ctk.CTkLabel(inner, text="Conectando con el servidor",
                         font=F(12), text_color=TM).pack()
            bar = ctk.CTkProgressBar(inner, width=220, height=6, corner_radius=3,
            ...
            ctk.CTkLabel(inner, text="Hola de nuevo", font=F(17, True),
                         text_color=TX).pack(pady=(12, 2))
            ctk.CTkLabel(inner, text="Usa la cuenta que recibiste con tu suscripción",
                         font=F(12), text_color=TM).pack(pady=(0, 16))

            def _foco(e):
                e.bind("<FocusIn>",  lambda *_: e.configure(border_color=ACCENT))
                e.bind("<FocusOut>", lambda *_: e.configure(border_color=BORDER))

            self._lock_email = ctk.CTkEntry(inner, width=300, height=42,
                                            corner_radius=12, border_width=1,
                                            placeholder_text="Correo",
                                            font=F(13), fg_color=CARD2,
                                            border_color=BORDER, text_color=TX)
            self._lock_email.pack(pady=4)
            _foco(self._lock_email)
            ult = licencia.ultimo_email()
            if ult:
                self._lock_email.insert(0, ult)
            self._lock_pass = ctk.CTkEntry(inner, width=300, height=42,
                                           corner_radius=12, border_width=1,
                                           placeholder_text="Contraseña", show="•",
                                           font=F(13), fg_color=CARD2,
                                           border_color=BORDER, text_color=TX)
            self._lock_pass.pack(pady=4)
            _foco(self._lock_pass)
            self._lock_pass.bind("<Return>", lambda *_: self._lic_login())
            # Mostrar/ocultar contraseña (ojo dentro del campo)
            def _ver_pw():
                oculta = self._lock_pass.cget("show") == "•"
                self._lock_pass.configure(show="" if oculta else "•")
                self._lock_eye.configure(text="🙈" if oculta else "👁")
            self._lock_eye = ctk.CTkButton(inner, text="👁", width=30, height=26,
                                           corner_radius=13, font=F(12),
                                           fg_color="transparent",
                                           hover_color=BORDER, text_color=TM,
                                           command=_ver_pw)
            self._lock_eye.place(in_=self._lock_pass, relx=1, rely=.5,
                                 x=-6, anchor="e")

            self._lock_btn = ctk.CTkButton(inner, text="Entrar", width=300,
                                           height=42, corner_radius=21,
                                           font=F(14, True), fg_color=ACCENT,
                                           hover_color=ACCENT_H,
                                           command=self._lic_login)
            self._lock_btn.pack(pady=(14, 0))
            hover_fluido(self._lock_btn, ACCENT, "#00D55F")

            # Separador "o"
            div = ctk.CTkFrame(inner, fg_color="transparent", width=300, height=18)
            div.pack(pady=(10, 4))
            div.pack_propagate(False)
            ctk.CTkFrame(div, fg_color=BORDER, height=1, corner_radius=0
                         ).place(relx=0, rely=.5, relwidth=.42)
            ctk.CTkLabel(div, text="o", font=F(11), text_color=TM
                         ).place(relx=.5, rely=.5, anchor="center")
            ctk.CTkFrame(div, fg_color=BORDER, height=1, corner_radius=0
                         ).place(relx=.58, rely=.5, relwidth=.42)

            self._lock_google = ctk.CTkButton(inner, text="G  Entrar con Google",
                                              width=300, height=42, corner_radius=21,
                                              font=F(13, True),
...
class BaseTab(ctk.CTkFrame):
    ...
    def _setup_ui(self):
        ...
        # ── Tarjeta 1 · Archivo ──────────────────────────────────────────────
        card = ctk.CTkFrame(self, fg_color=CARD, corner_radius=16, border_width=1, border_color=GLASS)
        ...
        hover_fluido(self.btn_select, ACCENT, "#00D55F")
        ...
        cfg = ctk.CTkFrame(self, fg_color=CARD, corner_radius=16, border_width=1, border_color=GLASS)
        ...
        # ── Tarjeta 3 · Ejecución ────────────────────────────────────────────
        run = ctk.CTkFrame(self, fg_color=CARD, corner_radius=16, border_width=1, border_color=GLASS)
        ...
        hover_fluido(self.btn_start, ACCENT, "#00D55F")
        ...
        hover_fluido(self.btn_stop, ERR, "#F0595E")
        ...
        hover_fluido(self.btn_download, CARD2, ("#DCDCE4", "#3A3A55"))
